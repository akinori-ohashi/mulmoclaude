...
| Variable                        | Used by                              | Notes                                                                                                                                                                                                                                                                         |
| ------------------------------- | ------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `DISCORD_SESSION_GRANULARITY`   | `@mulmobridge/discord`               | `thread` (default) or `channel`. A Discord thread is a channel with its own id, so `thread` keys a session by the thread and `channel` folds every thread into its parent channel's session. A thread is also allow-listed by its parent channel rather than its own id. Unrelated to `SLACK_SESSION_GRANULARITY`, which defaults the other way because a Slack thread is a facet of a channel, not a channel (#3217). |
