...
describe("optional AppSettings fields", () => {
  ...
  const pools = () => ({
    valid: {
      agentBackend: ["auto", "claude-code", "codex"],
...
describe("saveSettings", () => {
  ...
  it("persists chatModel and roundtrips it through loadSettings", () => {
    ...
    assert.deepEqual(mod.loadSettings(), { extraAllowedTools: [], agentBackend: "codex", chatModel: "sonnet" });
    mod.saveSettings({ extraAllowedTools: [], chatModel: "haiku" });
    assert.deepEqual(mod.loadSettings(), { extraAllowedTools: [], agentBackend: "codex", chatModel: "haiku" });
  });
