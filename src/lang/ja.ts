...
const jaMessages = {
  ...
  settingsModal: {
    ...
    agentTab: {
      description: "会話に使用するコマンドラインエージェントを選択します。変更は次のターンから反映されます。",
      backendLabel: "会話バックエンド",
      helperText: "自動は互換性のため Claude Code を優先し、利用できない場合は Codex を使います。Codex には認証済みの `codex` CLI が PATH 上に必要です。",
      configured: "バックエンド: {backend}",
      backend: {
        auto: "自動（Claude 優先）",
