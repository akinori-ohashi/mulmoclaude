 ...
 *
 * The env var leads because it works from every entry point without qualification.
 * The flag is equivalent everywhere as of #3113 — before that it was a silent
 * no-op on `yarn dev`, whose compound script handed trailing args to
 * `concurrently`, which dropped them.
 *
 * @param {number} port
 * @returns {string}
 */
export function instanceGuardMessage(port) {
  return [
    ...
    "  (--allow-multiple-instances is the same switch)",
  ].join("\n");
}
