# feat(#3147): セッション単位でモデルを一時上書きする

## 背景

#2923（全体）→ #2554（有効モデルの表示）→ #3104（ロール別）がマージ済み。
残るのは「普段は sonnet のロールだが、**この会話だけ** opus に上げたい」。
ロールだけでは重複ロールを作るしかなく、ロール一覧が用途ではなくモデルで増える。

## 方針

段を1つ増やすだけ。既存の解決点は1箇所なので、変更は素直に積める。

```
session  →  role  →  global  →  shared
```

| 変更 | 場所 |
|---|---|
| `SessionMeta.chatModel`（選んだ値） | `server/utils/files/session-io.ts` |
| `resolveChatModel` に引数追加 + `source: "session"` | `src/config/chatModelSource.ts` |
| 解決点に渡す | `server/agent/index.ts` |
| セッション設定の API | `server/api/routes/sessions.ts`（`bookmark` が前例） |
| チップから選ぶ UI | `src/components/SessionModelChip.vue` |

## 命名

- `SessionMeta.resolvedModel` — **観測値**。CLI の `system`/`init` が報告した実際のモデル（#2554）
- `SessionMeta.chatModel` — **設定値**。ユーザーがこのセッションに選んだ上書き（`AppSettings.chatModel` と同じ語）

観測と設定を名前で区別する。同じ語を使い回すと、どちらの話かが読めなくなる。

## UI

ピッカーを3つ並べない。#2554 で入れたチップを「有効な値 + 出所」の表示から
「そこから上書きもできる」コントロールに拡張する。

```
[⭐ Guide ▾]   [Sonnet 5 · ロールより ▾]
```

- 上書き中は見た目を変え、「既定に戻す」を出す
- セッション別は**一時上書き**であって対等な設定ではない

## 検証で外せない点

- **配線を固定する**。#3104 の Codex P3 と同じ穴を開けないこと。`buildAgentInput` を
  session 込みで駆動するテストを足し、session を無視する変異で赤くなることを確認する
- 実機で `--model` に session の値が乗ること（ground truth は spawn 引数と #2554 のチップ）
- 4段すべての優先順位（session > role > global > shared）

## スコープ外

- 進行中のターンの切り替え（次のターンから効く）
- ロール/全体設定 UI の変更

## クロスレビューで判明したこと（#3148）

段を1つ足す変更のつもりが、**session meta の sidecar そのものが持っていた 2 つの穴**を表に出した。
どちらも #3147 が作ったものではないが、#3147 が新しい書き手を足したことで踏みやすくなった。

1. **sidecar の read-modify-write が直列化されていない。** 9 つの更新関数がどれも
   「全体を読む → 1 フィールド変えて全体を書く」なので、2 つが重なると負けた側の
   フィールドが消える。計測: `chatModel` と `resolvedModel` の同時更新 40 回で、
   毎回どちらかが消えた。`mutateSessionMeta` に一本化し、ファイル単位で直列化した。
2. **セッション ID が検証されずにパスになる。** `metaRel` が `../` を正規化するので、
   ルートパラメータ由来の ID がそのまま届くとワークスペース内の任意ファイルに書ける。
   実際に `../../config/settings` でアプリ自身の設定ファイルを上書きできた。
   最初は「危険な書き手を列挙して塞ぐ」形にしたが、3ラウンド連続で「もう1つの書き手」が
   見つかった（9つの meta 更新関数 → `appendSessionLine`/`createSessionMeta` →
   `resultsFilePath`）。悪い形を列挙するルールには必ず「もう1つ」がある。
   そこで**ルールを反転**し、「安全でない ID はパスにならない」を1箇所で言うようにした:
   `metaRel` / `jsonlRel` が `string | null` を返す。読み・書き・追記・削除・パス取得が
   同時に閉じる。nullable にしたのは、一覧経路が「拒否」ではなく「スキップ」できるように
   するためと、**消費側をコンパイラに列挙させる**ため（外部の呼び出し元はちょうど3箇所）。
   当初は読み取り側を #3152 に分離する予定だったが、分離の理由（一覧経路の意味論）が
   反転によって上流で解決したため、この PR に取り込み #3152 は close した。

UI 側では、#2554 の「値が無ければ何も出さない」という表示の作法を、コントロールになった後も
そのまま引き継いでいたため、**最初のターンの前はピッカーが出なかった**。
セッション別モデルを一番選びたい瞬間がそこなので、表示条件は親（セッションがあるか）に移した。
