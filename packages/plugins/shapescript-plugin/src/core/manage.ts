...
export const LICENSE_REQUIRED_MESSAGE = `A public post is licensed under ${SHAPE_LICENSE_LABEL} (${SHAPE_LICENSE_URL}): anyone may share and adapt it, commercially too, with credit to the author. Ask the user whether they agree, and pass \`acceptLicense: true\` only once they have — or post it as a draft (\`published: false\`), which needs no agreement.`;
